...
interface NewInsightPageProps {
  params: {
    id: string; // 記事ID
  };
}

const NewInsightPage = ({ params }: NewInsightPageProps) => {
  const { id: articleId } = params;
  const router = useRouter();
  const { user, loading: authLoading } = useAuth();
  const { currentArticle, loading: articleLoading, error: articleError, fetchArticleById } = useArticles();
  const { loading: insightLoading, error: insightError, createInsight, clearError } = useInsights();
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // インサイトを作成
    const newInsight = await createInsight(insight);
    if (newInsight) {
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white shadow-sm rounded-lg overflow-hidden">
            <div className="p-6">
              <h1 className="text-2xl font-bold mb-6">新規インサイト</h1>
